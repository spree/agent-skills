...
**Carrier tracking webhook**: Spree owns `POST /api/v3/webhooks/fulfillments/:integration_id` (prefixed integration ID, no API key). Your `Spree::Integration` subclass implements `parse_webhook_event(raw_post, headers)`: verify the signature (raise `Spree::Integration::WebhookSignatureError` → 401, also when no secret is configured), return `{ tracking_code:, tracking_status:, estimated_delivery_at:, delivered_at:, details: }` or `nil` to ignore (→ 200). The controller finds the store-scoped `Spree::Delivery` by tracking number and runs `Deliveries::UpdateTracking`; a `delivered` status confirms receipt on the fulfillment. Map unknown carrier statuses to `'unknown'`.
