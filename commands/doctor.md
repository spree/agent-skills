...
3. **Env file** — read `.env` at the project root: `SECRET_KEY_BASE` present and non-empty, `SPREE_PORT` (default 3000 when absent). Note `SPREE_VERSION_TAG` if pinned. Flag a missing `.env` as the likely root cause for boot loops. Check the three Active Record encryption keys (`ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY`, `ACTIVE_RECORD_ENCRYPTION_DETERMINISTIC_KEY`, `ACTIVE_RECORD_ENCRYPTION_KEY_DERIVATION_SALT`) are present and non-empty — report only presence, never print the values. Missing → **warn** (webhook signing secrets, gateway customer IDs and OAuth tokens are stored in plaintext); remedy: `spree encryption init`, then recreate containers (`spree update`, or `spree dev` when ejected). On a classic app (no `.env`), check instead with `bin/rails runner 'puts %i[primary_key deterministic_key key_derivation_salt].map { |k| [k, Rails.configuration.active_record.encryption[k].present?] }.to_h.inspect'` — remedy: `bin/rails db:encryption:init`, store the values in env vars or credentials, and make sure `config/application.rb` copies them into `config.active_record.encryption`. If `apps/storefront/.env*` exists, check `SPREE_PUBLISHABLE_KEY` isn't still `pk_REPLACE_ME…` (remedy: `spree api-key create --type publishable`).
